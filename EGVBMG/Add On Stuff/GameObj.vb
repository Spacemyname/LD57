    ...
    Public Shared Sub DrawLetter(loc As Vector2, text As String, col As Color, effect As Integer, scale As Single)
        ...
        For q As Integer = 0 To text.Length - 1
            Select Case effect
                ...
                Case 15 'BG thicc fade
                    If q = 0 Then
                        Globals.SpriteBatch.Draw(Textures.Null, New Rectangle(loc.X + lnChar * 10 * scale + offset.X - 2, loc.Y + line * 14 * scale + offset.Y - 2, 11 * scale, 18 * scale), Color.Black * 0.4F)
                    ElseIf q = text.Length - 1 Then
                        Globals.SpriteBatch.Draw(Textures.Null, New Rectangle(loc.X + lnChar * 10 * scale + offset.X, loc.Y + line * 14 * scale + offset.Y - 2, 12 * scale, 18 * scale), Color.Black * 0.4F)
                    End If
                    Globals.SpriteBatch.Draw(Textures.Null, New Rectangle(loc.X + lnChar * 10 * scale + offset.X, loc.Y + line * 14 * scale + offset.Y - 2, 11 * scale, 18 * scale), Color.Black * 0.4F)
            End Select
